...
def choice():
    ...
    for i in range(0,7):
        print("Enter ",i+1,"for", choice_list[i],"\t \t \tEnter",i+6+1,"for", choice_list[(i+5)])
...
def analysis(group,selected_group,selected_member):
  member_sum_dict = {}
  print("Group Name : ",selected_group)
  df = group[selected_group]
  df_list = []
  member_list = list(df.columns)
  for name in member_list:
    df_member = df[df['paid by']== name]
    df.to_csv('{}.csv'.format(name))
    df_list.append(df_member)

  index_no = member_list.index(selected_member)

  for k in range(len(df_list)):
    sum_of_member = list(df_list[k].sum())
    member_sum_dict[member_list[k]] = sum_of_member

  selected_member_id = member_list.index(selected_member)
  new_member_list = member_list.copy()

  new_member_list.remove(selected_member)

  for i in new_member_list:
    result = (member_sum_dict[selected_member][member_list.index(i)] -  member_sum_dict[i][member_list.index(selected_member)])
    if result < 0:
      print(selected_member,"need to give ",result,"amount to ",i)
    elif result>0:
      print(selected_member,"need to take ",result,"amount to ",i)

def show_group(group):
  for i in list(group.keys()):
    print("group name : " , i)
  selected_goup = input("Enter your Selected group name : ")
  df = group[selected_goup]
  member_list = list(df.columns)
  ...
  analysis(group,selected_goup,selected_member)
...
def entry(group,selcted_goup):
    ...
    data =[0]*(len(member_list))

    for j in split_member_index:
        data[j-1] = split_amount

    for i,j in zip(member_list,data):
      df[i] = [j] 

    df['paid_by'] = [paid_by_name]
    df['Total'] = [amount]
...
def splite_Bills():
    group =  {}
    while True:
        print("Enter 1 to create entry to Group : ")
        print("Enter 2 to Create Group : ")
        print("Enter 3 to show group Trainsaction :")
        ...
        if choice_b == 0:
            ...
            print(group[selcted_goup])
            group = entry(group,selcted_goup)
        elif choice_b == 2:
          group = creat_group(group)
          print(group)
        elif choice_b == 3:
          show_group(group)
...
def percentage_calculator():
    while True:
        ...
        if choice_pp == 1: 
            ...
            print("Your Percentage : ",(obtain_marks/total_marks)*100,"%")
